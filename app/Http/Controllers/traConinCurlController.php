...
class traConinCurlController extends Controller
{
	public function TRALineD($seleStr='', $stationID=''){
        ...
        $seleTRA = $this->db->query("SELECT * FROM trastationd WHERE StationID='".$stationID."' && op=1");
        $seleTRAD = $seleTRA->fetchAll(PDO::FETCH_ASSOC);
        ...
        if($seleTRAD==null){
			...
			if($seleStr=='stationNow'){
				...
				return $this->TTRAD($seleStr, $url, $stationID);
			}
			else if($seleStr=='stationToday'){
                // \Log::info('stationToday');
				//echo 'stationToday';
    ...
    //	$timestamp=date("D, d M Y H:i:s GMT");//." GMT";
		...
		if($seleStr=='stationNow'){
			return $this->stationLineNow($result, $stationID);
		}
		else if($seleStr=='stationToday'){
			return $this->stationTodayData($result, $stationID);
		}
		else{
			return '404.0';
		}
	...
	private function stationLineNow($result, $stationID){
		#-------------------------------------
		#變數宣告
		#-------------------------------------
		$retD = array();
